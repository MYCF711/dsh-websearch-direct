...
**后果只是转移**：停用 A、删 A 后，**从未停用的 B 会突然被停用**。不是「丢失」，而是
**转移到错误的行** —— 用户操作的是 A，B 却莫名其妙变成停用状态。
...
```
test-v0.4-ui.mjs  → 38 通过 / 0 失败   🔴 对运行时停用缺陷零判别力
test-v3-model.mjs → 25 通过 / 1 失败   ❌ V3-5c 抓到
```

⇒ 当时**`test-v0.4-ui.mjs` 的 38 条断言对运行时停用机制无判别力**，
**V3-5c 是目前唯一护栏**。注意这不是用户可见 bug，是测试覆盖说明。
